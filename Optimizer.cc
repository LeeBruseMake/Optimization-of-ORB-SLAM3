...
float g_current_quality = 1.0f;   // 由主程序每帧更新的图像质量分数
float g_current_motion = 1.0f;   // 由主程序每帧更新的图像质量分数
       ...
       const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(leftIndex != -1 && pKF->mvuRight[get<0>(mit->second)]<0)
            {
                ...
                // 计算视觉缩放因子
                float quality = pKF->mQuality;
                float scale = 1.0f + 1.5f * (1.0f - quality);
                scale = std::min(scale,2.0f);
                //float scale = 1.0f;
                e->setInformation(Eigen::Matrix2d::Identity()*invSigma2 / scale);
            ...
            else if(leftIndex != -1 && pKF->mvuRight[leftIndex] >= 0) //Stereo observation
            {
                ...
                scale = std::min(scale,2.0f);
                //float scale = 1.0f;
                Eigen::Matrix3d Info = Eigen::Matrix3d::Identity()*invSigma2 / scale;
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(pKFi->mPrevKF && pKFi->mnId<=maxKFid)
        {
            ...
            if(pKFi->bImu && pKFi->mPrevKF->bImu)
            {
                ...
                //放缩
                float motion = pKFi->mMotion;
                if (motion < 0.1f) motion = 0.1f;
                float scale_imu =  1.0f + 1.0f * motion;
                //float scale_imu = 1.0f;
                ei->setInformation(ei->information() * scale_imu);
                //
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                ...
                else if(leftIndex != -1 && pKFi->mvuRight[leftIndex] >= 0) // stereo observation
                {
                    ...
                    scale = std::min(scale,2.0f);
                    //float scale = 1.0f;
                    e->setInformation(Eigen::Matrix3d::Identity()*invSigma2 / scale);
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    // 计算基于图像质量的缩放因子
    float quality = pFrame->mQuality;
    float scale = 1.0f + 1.5f * (1.0f - quality);
    scale = std::min(scale,2.0f);  // 质量越低，scale越大，信息矩阵越小
    //float scale = 1.0f;
    {
        ...
        if(pMP)
        {
            ...
            if(!pFrame->mpCamera2){
                ...
                else  // Stereo observation
                {
                    ...
                    const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
                    //Eigen::Matrix3d Info = Eigen::Matrix3d::Identity()*invSigma2;
                    Eigen::Matrix3d Info = Eigen::Matrix3d::Identity() * invSigma2 / scale;   // 同样除以 scal
                    e->setInformation(Info);
...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    Map* pCurrentMap = pKF->GetMap();
    //打印调试信息
    cout << "LBA_READ: KF " << pKF->mnId << " reading quality = " << g_current_quality << endl;

    //int maxOpt=10;
    int opt_it=10;

    // ---------- 新增：根据机动性动态调整 maxOpt ----------
    float motion = pKF->mMotion;

    int baseOpt = 8;
    int maxOptRange = 15;
    int minOptRange = 8;
    int maxOpt = static_cast<int>(round(baseOpt + (10 * motion)));
    maxOpt = std::min(maxOptRange,maxOpt);
    ...
    */
    //打印调试信息
    cout << "LocalBA: KF " << pKF->mnId << ", motion=" << motion << ", maxOpt=" << maxOpt << endl;
    //
    ...
    for(int i=0;i<N;i++)
    {
        ...
        if(pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated)
        {
            ...
            float scale_imu = 1.0f + 1.0f * motion;  
            //float scale_imu = 1.0f;
            vei[i]->setInformation(vei[i]->information() * scale_imu);
            //
