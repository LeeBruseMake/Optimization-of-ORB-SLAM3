...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::STEREO && !mpCamera2)
    {
        ...
        mCurrentFrame.mMotion = g_current_motion;   // 新增
        mCurrentFrame.mQuality = g_current_quality;  // 新增
    }
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // 获取当前帧机动性
    float motion = mCurrentFrame.mMotion;
    ...
    else
    {
        ...
        else if(mpCamera2) thRefRatio = 0.75f;
    ...
    float k = 0.2f;   // 可调参数，控制机动性影响程度
    thRefRatio = thRefRatio / (1.0f + k * motion);

    // 限制阈值范围，避免过小或过大
    if(thRefRatio < 0.4f) thRefRatio = 0.4f;
    if(thRefRatio > 0.95f) thRefRatio = 0.95f;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    pKF->mMotion = mCurrentFrame.mMotion;//新增
    cout << "New KeyFrame " << pKF->mnId << " created with motion = " << pKF->mMotion << endl;//新增
    ...
    //打印调试信息
    cout << "CREATE_KF: KF " << pKF->mnId << " created with quality = " << g_current_quality << endl;
